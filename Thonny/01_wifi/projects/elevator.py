...
class Elevator():
    ...
    def __vl53l0x_thread(self):
        try:
            laser = LaserRanging(gpio1 = Pin(35, Pin.IN), scl=Pin(33), sda=Pin(32), freq=100000)
            laser.start()
            ...
            self.allowable_floor_error = 0.05
            print(f"distance:{self.distance},floor:{self.floor}")
            while not self.stop_thread:
                self.distance = laser.read()
                #print("{:>4}mm".format(self.distance))
                self.floor = self.__get_current_floor(self.distance)
                time.sleep_ms(500)
        except KeyboardInterrupt:
            string = "KeyboardInterrupt (Ctrl+C) received."
            print(string)
        except Exception as e:
            time.sleep_ms(2000)
            self.lcd1602.putstr("{}".format(e), 0, 0, clear = True)
        self.stop_thread = True
        
            
    def __matrix_8x8_display_thread(self):
        spi = SoftSPI(sck=Pin(25), mosi=Pin(27), miso = Pin(12))
        cs = Pin(26, Pin.OUT)
    ...
    def __on_off_4x4_thread(self):
        ...
        while not self.stop_thread:
            ...
            if key is not None and key <= 6:
                ...
                text = f"Press key:{self.matrix_4x4_press_key}"
                print(text)
            time.sleep_ms(200)
            
    def __lcd1602_thread(self):
        self.lcd1602.putstr("123456789ABCDEFG", 0, 0, clear = True)
        self.lcd1602.putstr("^", math.floor(self.floor + 0.5) - 1, 1)
        old_key = self.matrix_4x4_press_key
        while not self.stop_thread:
            if self.__is_target_floor_arrived() == False and self.matrix_4x4_press_key != old_key:
                old_key = self.matrix_4x4_press_key
                old_floor = 0
                while not self.stop_thread and self.__is_target_floor_arrived() == False:
                    new_floor = math.floor(self.floor + 0.5)
                    if old_floor != new_floor:
                        old_floor = new_floor
                        print(f"new_floor:{new_floor}")
                        self.lcd1602.putstr("123456789ABCDEFG", 0, 0, clear = True)
                        if self.floor < self.target_floor:
                            self.lcd1602.putstr(">", new_floor - 1, 1, clear = False )
                        elif self.floor > self.target_floor:
                            self.lcd1602.putstr("<", new_floor - 1, 1, clear = False )
                        time.sleep_ms(500)
                self.lcd1602.putstr("123456789ABCDEFG", 0, 0, clear = True)
                self.lcd1602.putstr("^", self.target_floor - 1, 1, clear = False )
            time.sleep_ms(500)
    ...
    def run(self):
        ...
        thread_lcd1602 = _thread.start_new_thread(self.__lcd1602_thread, ())
